 ...
 *   No additional logic, side effects, or parameter changes are introduced here.
 */
bool ParticleID::NeutralECALveto(std::vector<clas12::region_part_ptr>& allParticles, std::vector<clas12::region_part_ptr>& electrons, const double& beamE, clas12::region_part_ptr neut,
                                 const double& veto_cut) {
    // Forward all arguments verbatim to the implementation in `reco_analysis_functions`.
...
void ParticleID::FDNeutralParticleID(std::vector<clas12::region_part_ptr> allParticles, std::vector<clas12::region_part_ptr> electrons, std::vector<int>& FD_Neutrons_within_PID_cuts,
                                     std::vector<int>& ID_Neutrons_FD, DSCuts& Neutron_momentum_th, std::vector<int>& FD_Photons_within_th, std::vector<int>& ID_Photons_FD, DSCuts& Photon_momentum_th,
 ...
 * Neutron = a neutral particle (i.e., neutron or photon) in the FD with no PCal hit and with an ECal hit.
 * Photon = a neutral particle (i.e., neutron or photon) in the FD with a PCal hit. */
void ParticleID::FDNeutralParticleID(std::vector<clas12::region_part_ptr> allParticles, std::vector<int>& FD_Neutrons_within_th, std::vector<int>& ID_Neutrons_FD, DSCuts& Neutron_momentum_th,
...
int ParticleID::GetLnFDIndex(std::vector<clas12::region_part_ptr> allParticles, std::vector<int>& FD_Neutrons_within_th, const bool& apply_nucleon_cuts) {
    bool PrintOut = false;
...
int ParticleID::GetCorrLnFDIndex(MomentumResolution& nRes, std::vector<clas12::region_part_ptr> allParticles, std::vector<int>& FD_Neutrons_within_th, const bool& apply_nucleon_cuts,
...
std::vector<int> ParticleID::GetFDNeutrons(std::vector<clas12::region_part_ptr>& allParticles, const DSCuts& Momentum_cuts, const bool& apply_nucleon_cuts) {
...
std::vector<int> ParticleID::GetGoodProtons(const bool& apply_nucleon_cuts, std::vector<clas12::region_part_ptr>& protons, const std::vector<int>& IDProtons_ind, const DSCuts& Theta_p1_cuts_2p,
...
void ParticleID::FillNeutMultiPlots(std::vector<clas12::region_part_ptr>& allParticles, std::vector<clas12::region_part_ptr>& electrons, const double& Weight, const double& beamE,
                                    const double& ECAL_veto_cut, hPlot1D& hNeutronMulti_BPID_BV, hPlot1D& hNeutronMulti_BPID_AV, std::vector<int>& Neutrons_BPID, hPlot1D& hNeutronMulti_APID_BV,
                                    hPlot1D& hNeutronMulti_APID_AV, std::vector<int>& Neutrons_APID) {
    FillMultiPlots(allParticles, electrons, Weight, beamE, ECAL_veto_cut, hNeutronMulti_BPID_BV, hNeutronMulti_BPID_AV, Neutrons_BPID);
    FillMultiPlots(allParticles, electrons, Weight, beamE, ECAL_veto_cut, hNeutronMulti_APID_BV, hNeutronMulti_APID_AV, Neutrons_APID);
}
