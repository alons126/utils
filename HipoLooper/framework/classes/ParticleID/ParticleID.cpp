 ...
 */
bool ParticleID::NeutronECAL_Cut_Veto(vector<clas12::region_part_ptr>& allParticles, vector<clas12::region_part_ptr>& electrons, const double& beamE, const int& index,
 ...
 *   No additional logic, side effects, or parameter changes are introduced here.
 */
bool ParticleID::ChargedECALveto(vector<clas12::region_part_ptr>& allParticles, clas12::region_part_ptr& electron, clas12::region_part_ptr NeutronFD, const double& veto_cut,
 ...
 *   No additional logic, side effects, or parameter changes are introduced here.
 */
bool ParticleID::NeutralECALveto(vector<clas12::region_part_ptr>& allParticles, vector<clas12::region_part_ptr>& electrons, const double& beamE, clas12::region_part_ptr neut,
                                 const double& veto_cut) {
    // Forward all arguments verbatim to the implementation in `reco_analysis_functions`.
    return raf::NeutralECALveto(allParticles, electrons, beamE, neut, veto_cut);
}
#pragma endregion
...
vector<int> ParticleID::ChargedParticleID(vector<clas12::region_part_ptr>& ChargedParticles, const DSCuts& FD_mom_th, const DSCuts& CD_mom_th) {
...
vector<int> ParticleID::ChargedParticleID(vector<clas12::region_part_ptr>& ChargedParticles, const DSCuts& Momentum_th) {
    vector<int> ChargedParticles_ind;
...
void ParticleID::FDNeutralParticleID(vector<clas12::region_part_ptr> allParticles, vector<clas12::region_part_ptr> electrons,             // clas12::region_part_ptr elements
                                     vector<int>& FD_Neutrons_within_PID_cuts, vector<int>& ID_Neutrons_FD, DSCuts& Neutron_momentum_th,  // FD neutrons
                                     vector<int>& FD_Photons_within_th, vector<int>& ID_Photons_FD, DSCuts& Photon_momentum_th,           // FD photons
    ...
    for (int& i : ID_Neutrons_FD) {  // Identify neutron above momentum threshold
        ...
        double Momentum = raf::CalcPnFD(allParticles[i], electrons[0], starttime, apply_nucleon_cuts);
...
#pragma endregion
        ...
        if (apply_nucleon_cuts) {
            // NeutronPassesChargedECALVeto = NeutronECAL_Cut_Veto(allParticles, electrons, beamE, i, Neutron_veto_cut.GetLowerCutConst());
            ...
            NeutronPassesNeutralECALVeto = this->NeutralECALveto(allParticles, electrons, beamE, allParticles[i], NeutralECALveto_veto_cut.GetLowerCutConst());
 ...
 * Neutron = a neutral particle (i.e., neutron or photon) in the FD with no PCal hit and with an ECal hit.
 * Photon = a neutral particle (i.e., neutron or photon) in the FD with a PCal hit. */
void ParticleID::FDNeutralParticleID(vector<clas12::region_part_ptr> allParticles, vector<clas12::region_part_ptr> electrons, vector<int>& FD_Neutrons_within_PID_cuts,
                                     vector<int>& ID_Neutrons_FD, DSCuts& Neutron_momentum_th, vector<int>& FD_Photons_within_th, vector<int>& ID_Photons_FD, DSCuts& Photon_momentum_th,
                                     DSCuts& Neutron_veto_cut, const double& beamE, const double& ECAL_V_edge_cut, const double& ECAL_W_edge_cut, const bool& apply_nucleon_cuts) {
    for (int& i : ID_Neutrons_FD) {  // Identify neutron above momentum threshold
        /* Particles that get in here are neutrons. Now we take neutrons who pass momentum cuts. */

        int NeutralPDG = allParticles[i]->par()->getPid();
        double Momentum = this->GetFDNeutronP(allParticles[i], apply_nucleon_cuts);
 ...
 * Neutron = a neutral particle (i.e., neutron or photon) in the FD with no PCal hit and with an ECal hit.
 * Photon = a neutral particle (i.e., neutron or photon) in the FD with a PCal hit. */
void ParticleID::FDNeutralParticleID(vector<clas12::region_part_ptr> allParticles, vector<int>& FD_Neutrons_within_th, vector<int>& ID_Neutrons_FD, DSCuts& Neutron_momentum_th,
...
int ParticleID::GetLnFDIndex(vector<clas12::region_part_ptr> allParticles, vector<int>& FD_Neutrons_within_th, const bool& apply_nucleon_cuts) {
    bool PrintOut = false;
    ...
    for (int& i : FD_Neutrons_within_th) {  // Identify the leading neutron
        double P_temp = this->GetFDNeutronP(allParticles[i], apply_nucleon_cuts);
...
int ParticleID::GetCorrLnFDIndex(MomentumResolution& nRes, vector<clas12::region_part_ptr> allParticles, vector<int>& FD_Neutrons_within_th, const bool& apply_nucleon_cuts,
    ...
    for (int i = 0; i < FD_Neutrons_within_th.size(); i++) {
        double P_temp = this->GetFDNeutronP(allParticles[FD_Neutrons_within_th.at(i)], apply_nucleon_cuts);
...
#pragma endregion
    ...
    if (PrintLog) {
        for (int i = 0; i < FD_Neutrons_within_th.size(); i++) {
            double P_temp0 = this->GetFDNeutronP(allParticles[FD_Neutrons_within_th.at(i)], apply_nucleon_cuts);
...
void ParticleID::ReDefFDNeutrals(vector<clas12::region_part_ptr> allParticles, vector<int>& ReDef_FD_neutrons, vector<int>& ReDef_FD_photons) {
...
double ParticleID::GetFDNeutronP(clas12::region_part_ptr& Neutron, const bool& apply_nucleon_cuts) {
    double Momentum;
...
vector<int> ParticleID::GetFDNeutrons(vector<clas12::region_part_ptr>& allParticles, const DSCuts& Momentum_cuts, const bool& apply_nucleon_cuts) {
...
vector<int> ParticleID::GetFDPhotons(vector<clas12::region_part_ptr>& allParticles, const DSCuts& Momentum_cuts) {
...
vector<int> ParticleID::GetGoodParticles(vector<clas12::region_part_ptr>& Particle,  // particle
...
vector<int> ParticleID::GetGoodProtons(const bool& apply_nucleon_cuts, vector<clas12::region_part_ptr>& protons, const vector<int>& IDProtons_ind, const DSCuts& Theta_p1_cuts_2p,
...
void ParticleID::GPMonitoring(const bool& GoodProtonsMonitorPlots, vector<clas12::region_part_ptr>& protons, const vector<int>& IDProtons_ind, const vector<int>& Protons_ind,
...
void ParticleID::nParticleID(vector<clas12::region_part_ptr>& allParticles, vector<int>& ID_Neutrons_FD, const DSCuts& Neutron_momentum_th, vector<int>& ID_Photons_FD,
...
ParticleID::ParticleVectorsTuple ParticleID::SetEventParticles(const bool& clas12ana_particles, clas12ana& clasAna, const std::unique_ptr<clas12::clas12reader>& c12) {
    ...
    vector<clas12::region_part_ptr> allParticles;
...
void ParticleID::FillNeutMultiPlots(vector<clas12::region_part_ptr>& allParticles, vector<clas12::region_part_ptr>& electrons, const double& Weight, const double& beamE,
                                    const double& ECAL_veto_cut, hPlot1D& hNeutronMulti_BPID_BV, hPlot1D& hNeutronMulti_BPID_AV, vector<clas12::region_part_ptr>& Neutrons_BPID,
                                    hPlot1D& hNeutronMulti_APID_BV, hPlot1D& hNeutronMulti_APID_AV, vector<clas12::region_part_ptr>& Neutrons_APID) {
...
void ParticleID::FillMultiPlots(vector<clas12::region_part_ptr>& allParticles, vector<clas12::region_part_ptr>& electrons, const double& Weight, const double& beamE,
                                const double& ECAL_veto_cut, hPlot1D& hNeutronMulti_BV, hPlot1D& hNeutronMulti_AV, vector<clas12::region_part_ptr>& Neutrons) {
...
void ParticleID::FillNeutMultiPlots(vector<clas12::region_part_ptr>& allParticles, vector<clas12::region_part_ptr>& electrons, const double& Weight, const double& beamE,
                                    const double& ECAL_veto_cut, hPlot1D& hNeutronMulti_BPID_BV, hPlot1D& hNeutronMulti_BPID_AV, vector<int>& Neutrons_BPID, hPlot1D& hNeutronMulti_APID_BV,
