...
class Debugger {
   ...
   public:
    ...
    void SafetyCheck_clas12ana_particles(const char* FILE, const int LINE, const bool& clas12ana_particles, std::vector<region_part_ptr>& allParticles, const int Nf);

    // SafetyCheck_FD_protons function ------------------------------------------------------------------------------------------------------------------------------------------------

    void SafetyCheck_FD_protons(const char* FILE, const int LINE, const vector<int>& Protons_ind, std::vector<region_part_ptr>& protons, const DSCuts& pFD_mom_th);
    ...
    void SafetyCheck_Truth_leading_FD_neutron(const char* FILE, const int LINE, const bool& ES_by_leading_FDneutron, const int& TL_IDed_Leading_nFD_ind,
                                              const double& TL_IDed_Leading_nFD_momentum, const double& Leading_TL_FDNeutron_Momentum, vector<int> TL_NeutronsFD_mom_ind, mcpar_ptr mcpbank);

    // SafetyCheck_FD_neutron function ------------------------------------------------------------------------------------------------------------------------------------------------

    void SafetyCheck_FD_neutron(const char* FILE, const int LINE, const bool& apply_nucleon_cuts, std::vector<region_part_ptr>& allParticles, const DSCuts& n_mom_th,
    ...
    void SafetyCheck_AMaps_Reco_leading_neutrons(const char* FILE, const int LINE, std::vector<region_part_ptr>& allParticles, const int& NeutronsFD_ind_mom_max, const bool& hitPCAL_1e_cut,
                                                 const bool& hitECIN_1e_cut, const bool& hitECOUT_1e_cut);

    // SafetyCheck_one_good_electron function ------------------------------------------------------------------------------------------------------------------------------------------------

    void SafetyCheck_one_good_electron(const char* FILE, const int LINE, std::vector<region_part_ptr>& electrons);

    // SafetyCheck_1e_cut_electron function ------------------------------------------------------------------------------------------------------------------------------------------------

    void SafetyCheck_1e_cut_electron(const char* FILE, const int LINE, std::vector<region_part_ptr>& electrons, const vector<int>& Electron_ind);
    ...
    void SafetyCheck_1p(const char* FILE, const int LINE, const vector<int>& Protons_ind, region_part_ptr& e_1p, region_part_ptr& p_1p, const bool& Enable_FD_photons,
                        vector<int>& PhotonsFD_ind);

    // SafetyCheck_1n function ------------------------------------------------------------------------------------------------------------------------------------------------

    void SafetyCheck_1n(const char* FILE, const int LINE, vector<int>& NeutronsFD_ind, region_part_ptr& e_1n, region_part_ptr& n_1n, const bool& Enable_FD_photons,
        ...
        if (DebuggerMode) {
            if (OnlyPrintNamedTesterSteps) {
                if (StepName != "") {
                    ...
                    std::cerr << env::ERROR_COLOR << "File:" << env::RESET_COLOR << " " << FILE << env::ERROR_COLOR << ", Line:" << env::RESET_COLOR << " " << LINE << env::ERROR_COLOR
                ...
                if (StepName == "") {
                    std::cerr << env::ERROR_COLOR << "File:" << env::RESET_COLOR << " " << FILE << env::ERROR_COLOR << ", Line:" << env::RESET_COLOR << " " << LINE << "\n\n"
                              << env::RESET_COLOR << "" << std::endl;
                } else {
                    std::cerr << env::ERROR_COLOR << "File:" << env::RESET_COLOR << " " << FILE << env::ERROR_COLOR << ", Line:" << env::RESET_COLOR << " " << LINE << env::ERROR_COLOR
                              << ", Step: " << env::RESET_COLOR << "" << StepName << "\n\n"
