...
void Debugger::SafetyCheck_clas12ana_particles(const char* FILE, const int LINE, const bool& clas12ana_particles, std::vector<region_part_ptr>& allParticles, const int Nf) {
...
void Debugger::SafetyCheck_Reco_leading_FD_neutron(const char* FILE, const int LINE, const bool& apply_nucleon_cuts, const bool& ES_by_leading_FDneutron, const int& NeutronsFD_ind_mom_max,
    ...
    if (ES_by_leading_FDneutron) {
        ...
        if (NeutronsFD_ind_mom_max != -1) {
            ...
            if (allParticles[NeutronsFD_ind_mom_max]->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "Leading reco nFD check: Leading nFD is not in the FD!", ""); }
...
void Debugger::SafetyCheck_AMaps_Truth_electrons(const char* FILE, const int LINE, const int& particlePDGtmp, const bool& inFD_AMaps) {
    if (particlePDGtmp != 11) { this->PrintErrorMessage(FILE, LINE, "TL electrons check (AMaps & WMaps): TL electron PGD is invalid (" + to_string(particlePDGtmp) + ")!", ""); }
...
void Debugger::SafetyCheck_AMaps_Truth_protons(const char* FILE, const int LINE, const int& particlePDGtmp, const bool& inFD_AMaps) {
    if (particlePDGtmp != 2212) { this->PrintErrorMessage(FILE, LINE, "TL protons check (AMaps & WMaps): TL proton PGD is invalid (" + to_string(particlePDGtmp) + ")!", ""); }
...
void Debugger::SafetyCheck_AMaps_Truth_neutrons(const char* FILE, const int LINE, const int& particlePDGtmp, const bool& inFD_AMaps) {
    if (particlePDGtmp != 2112) { this->PrintErrorMessage(FILE, LINE, "TL neutrons check (AMaps & WMaps): TL neutron PGD is invalid (" + to_string(particlePDGtmp) + ")!", ""); }

    if (!inFD_AMaps) { this->PrintErrorMessage(FILE, LINE, "TL neutrons check (AMaps & WMaps): TL neutron is not in FD!", ""); }
}

void Debugger::SafetyCheck_AMaps_Reco_leading_neutrons(const char* FILE, const int LINE, std::vector<region_part_ptr>& allParticles, const int& NeutronsFD_ind_mom_max,
                                                       const bool& hitPCAL_1e_cut, const bool& hitECIN_1e_cut, const bool& hitECOUT_1e_cut) {
    if (allParticles[NeutronsFD_ind_mom_max]->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "Leading reco nFD check (AMaps & WMaps): Leading nFD is not in the FD!", ""); }
...
void Debugger::SafetyCheck_one_good_electron(const char* FILE, const int LINE, std::vector<region_part_ptr>& electrons) {
...
void Debugger::SafetyCheck_1e_cut_electron(const char* FILE, const int LINE, std::vector<region_part_ptr>& electrons, const vector<int>& Electron_ind) {
    ...
    if (electrons[0]->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "1e cut: electrons is not in the FD!", ""); }
}
...
void Debugger::SafetyCheck_1p(const char* FILE, const int LINE, const vector<int>& Protons_ind, region_part_ptr& e_1p, region_part_ptr& p_1p, const bool& Enable_FD_photons,
    ...
    if (e_1p->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "1p: 1p proton is not in the FD!", ""); }

    if (p_1p->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "1p: 1p electron is not in the FD!", ""); }

    if (!(Enable_FD_photons || (PhotonsFD_ind.size() == 0))) { this->PrintErrorMessage(FILE, LINE, ("1p: PhotonsFD_ind.size() is non-zero (" + to_string(PhotonsFD_ind.size()) + ")!"), ""); }
}
...
void Debugger::SafetyCheck_1n(const char* FILE, const int LINE, vector<int>& NeutronsFD_ind, region_part_ptr& e_1n, region_part_ptr& n_1n, const bool& Enable_FD_photons,
    ...
    if (e_1n->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "1n: Electron is not in the FD!", ""); }

    if (n_1n->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "1n: nFD is not in the FD!", ""); }

    if (!(Enable_FD_photons || (PhotonsFD_ind.size() == 0))) { this->PrintErrorMessage(FILE, LINE, ("1n: PhotonsFD_ind.size() is non-zero (" + to_string(PhotonsFD_ind.size()) + ")!"), ""); }
    ...
    for (int& i : NeutronsFD_ind) {
        ...
        if (!((allParticles[i]->par()->getPid() == 2112) || (allParticles[i]->par()->getPid() == 22))) {
            this->PrintErrorMessage(FILE, LINE, ("1n: A neutron PDG is not 2112 or 22 (" + to_string(allParticles[i]->par()->getPid()) + ")!"), "");
        }
    }
    ...
    for (int& i : PhotonsFD_ind) {
        ...
        if (allParticles[i]->par()->getPid() != 22) {
            this->PrintErrorMessage(FILE, LINE, ("1n: A photon PDG is not 2112 or 22 (" + to_string(allParticles[i]->par()->getPid()) + ")!"), "");
        }
    }
    ...
    if (n_1n->getRegion() != FD) { this->PrintErrorMessage(FILE, LINE, "1n: neutron is not in FD!", ""); }

    if (!((NeutronPDG == 22) || (NeutronPDG == 2112))) { this->PrintErrorMessage(FILE, LINE, ("1n: neutral PDG is not 2112 or 22 (" + to_string(NeutronPDG) + ")!"), ""); }
