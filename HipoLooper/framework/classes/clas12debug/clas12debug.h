...
class clas12debug {
   ...
   private:
    ...
    std::unique_ptr<TH2D> pid_proton_tof_fd_b_debug =
        std::make_unique<TH2D>("pid_proton_tof_fd_b_debug", "chi2PID Proton FD;Momentum (GeV/c);TOF_{Measured}-TOF_{Expected} (ns)", 1000, 0, 3.5, 100, -1, 1);
    std::unique_ptr<TH2D> pid_proton_tof_cd_b_debug =
        std::make_unique<TH2D>("pid_proton_tof_cd_b_debug", "chi2PID Proton CD;Momentum (GeV/c);TOF_{Measured}-TOF_{Expected} (ns)", 1000, 0, 3.5, 100, -1, 1);

    std::unique_ptr<TH2D> pid_proton_tof_fd_a_debug =
        std::make_unique<TH2D>("pid_proton_tof_fd_a_debug", "chi2PID Proton FD;Momentum (GeV/c);TOF_{Measured}-TOF_{Expected} (ns)", 1000, 0, 3.5, 100, -1, 1);
    std::unique_ptr<TH2D> pid_proton_tof_cd_a_debug =
        std::make_unique<TH2D>("pid_proton_tof_cd_a_debug", "chi2PID Proton CD;Momentum (GeV/c);TOF_{Measured}-TOF_{Expected} (ns)", 1000, 0, 3.5, 100, -1, 1);
    ...
    std::unique_ptr<TH2D> cd_particles_b =
        std::make_unique<TH2D>("cd_edge_before", "CD protons before edge cut;#Phi angle (deg);Transverse Momentum P_t (GeV/c)", 100, -180, 180, 100, 0, 1.);
