...
void clas12debug::fillBeforePart(const clas12::region_part_ptr &p) {
    ...
    if (pid == 2212) fillDCdebug(p, dc_hit_map_b_proton);
    if (pid == 211) fillDCdebug(p, dc_hit_map_b_pion);
...
void clas12debug::fillAfterPart(const clas12::region_part_ptr &p) {
    ...
    if (p->par()->getCharge() >= 1 && p->par()->getPid() != 11 && p->par()->getPid() == 2212) {
        debugByPid(p);
        if (pid == 2212) fillDCdebug(p, dc_hit_map_a_proton);
        if (pid == 211) fillDCdebug(p, dc_hit_map_a_pion);
