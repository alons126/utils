...
// Wed Nov  3 05:52:53 2021 by ROOT version 6.18/04
// from TTree gst/GENIE Summary Event Tree
// found on file: 12C_2222GeV_G18_10a_02_11a_Q2_0_1.root
//////////////////////////////////////////////////////////
...
class gst {
   public:
    ...
    gst(const char *files, TTree *tree = 0);
    //    gst(TTree *tree=0);
    virtual ~gst();
    ...
    virtual void Loop(string Target, string GENIE_tune, string BeamE, string Q2_th);
...
#ifdef gst_cxx
gst::gst(const char *files, TTree *tree) : fChain(0) {
    std::string files_str(files);

    if (!utilities::FindSubstring(files_str, "*")) {
...
//       if (!f || !f->IsOpen()) {
//          f = new TFile("12C_2222GeV_G18_10a_02_11a_Q2_0_1.root");
//       }
//       f->GetObject("gst",tree);
...
void gst::Init(TTree *tree) {
    ...
    // (once per file to be FillHistogramByProcessed).
