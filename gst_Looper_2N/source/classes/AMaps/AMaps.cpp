...
AMaps::AMaps(const string &AcceptanceMapsDirectory, const string &SampleName, const bool &Electron_single_slice_test, const bool &Nucleon_single_slice_test, const vector<int> &TestSlices) {
    ...
    LoadAMapLimits((AcceptanceMapsDirectory + SampleName + "/e_AMap_by_slice/e_slice_limits.par").c_str(), Loaded_ElectronMomSliceLimits);
    LoadAMapLimits((AcceptanceMapsDirectory + SampleName + "/p_AMap_by_slice/p_slice_limits.par").c_str(), Loaded_NucleonMomSliceLimits);

    /* Load separate maps */
    LoadAMapSlices(SampleName, AcceptanceMapsDirectory, "Electron", Loaded_ElectronMomSliceLimits, Loaded_e_AMap_Slices);
    ReadWMapSlices(SampleName, AcceptanceMapsDirectory, "Electron", Loaded_ElectronMomSliceLimits, Loaded_e_WMap_Slices);
    LoadAMapSlices(SampleName, AcceptanceMapsDirectory, "Proton", Loaded_NucleonMomSliceLimits, Loaded_p_AMap_Slices);
    ReadWMapSlices(SampleName, AcceptanceMapsDirectory, "Proton", Loaded_NucleonMomSliceLimits, Loaded_p_WMap_Slices);
    LoadAMapSlices(SampleName, AcceptanceMapsDirectory, "Neutron", Loaded_NucleonMomSliceLimits, Loaded_n_AMap_Slices);
    ReadWMapSlices(SampleName, AcceptanceMapsDirectory, "Neutron", Loaded_NucleonMomSliceLimits, Loaded_n_WMap_Slices);
    LoadAMapSlices(SampleName, AcceptanceMapsDirectory, "Nucleon", Loaded_NucleonMomSliceLimits, Loaded_nuc_AMap_Slices);

    /* Load combined maps */
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName + "/e_AMap_file.par").c_str(), Loaded_e_AMap);
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName + "/p_AMap_file.par").c_str(), Loaded_p_AMap);
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName + "/n_AMap_file.par").c_str(), Loaded_n_AMap);
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName + "/nuc_AMap_file.par").c_str(), Loaded_nuc_AMap);
...
// Next-Gen AMaps loading constructor:
AMaps::AMaps(const string &AcceptanceMapsDirectory, const string &BeamE) {
    string SampleName_1e = "Uniform_1e_sample_" + BeamE, SampleName_en = "Uniform_en_sample_" + BeamE, SampleName_ep = "Uniform_ep_sample_" + BeamE;

    /* Load slices and their limits */
    LoadAMapLimits((AcceptanceMapsDirectory + SampleName_1e + "/e_AMap_by_slice/e_slice_limits.par").c_str(), Loaded_ElectronMomSliceLimits);
    LoadAMapLimits((AcceptanceMapsDirectory + SampleName_ep + "/p_AMap_by_slice/p_slice_limits.par").c_str(), Loaded_NucleonMomSliceLimits);
    // LoadAMapLimits((AcceptanceMapsDirectory + SampleName + "/e_AMap_by_slice/e_slice_limits.par").c_str(), Loaded_ElectronMomSliceLimits);
    // LoadAMapLimits((AcceptanceMapsDirectory + SampleName + "/p_AMap_by_slice/p_slice_limits.par").c_str(), Loaded_NucleonMomSliceLimits);

    /* Load separate maps */
    LoadAMapSlices(SampleName_1e, AcceptanceMapsDirectory, "Electron", Loaded_ElectronMomSliceLimits, Loaded_e_AMap_Slices);
    // ReadWMapSlices(SampleName, AcceptanceMapsDirectory, "Electron", Loaded_ElectronMomSliceLimits, Loaded_e_WMap_Slices);
    LoadAMapSlices(SampleName_ep, AcceptanceMapsDirectory, "Proton", Loaded_NucleonMomSliceLimits, Loaded_p_AMap_Slices);
    // ReadWMapSlices(SampleName, AcceptanceMapsDirectory, "Proton", Loaded_NucleonMomSliceLimits, Loaded_p_WMap_Slices);
    LoadAMapSlices(SampleName_en, AcceptanceMapsDirectory, "Neutron", Loaded_NucleonMomSliceLimits, Loaded_n_AMap_Slices);
    // ReadWMapSlices(SampleName, AcceptanceMapsDirectory, "Neutron", Loaded_NucleonMomSliceLimits, Loaded_n_WMap_Slices);
    // LoadAMapSlices(SampleName, AcceptanceMapsDirectory, "Nucleon", Loaded_NucleonMomSliceLimits, Loaded_nuc_AMap_Slices);

    /* Load combined maps */
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName_1e + "/e_AMap_file.par").c_str(), Loaded_e_AMap);
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName_ep + "/p_AMap_file.par").c_str(), Loaded_p_AMap);
    LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName_en + "/n_AMap_file.par").c_str(), Loaded_n_AMap);
    // LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName + "/nuc_AMap_file.par").c_str(), Loaded_nuc_AMap);

    HistElectronSliceNumOfXBins = Loaded_e_AMap.size();
    HistElectronSliceNumOfYBins = Loaded_e_AMap.size();
    HistNucSliceNumOfXBins = Loaded_p_AMap.size();
    HistNucSliceNumOfYBins = Loaded_p_AMap.size();
    // HistElectronSliceNumOfXBins = 100;  // 100 by Default
    // HistElectronSliceNumOfYBins = 100;  // 100 by Default
    // HistNucSliceNumOfXBins = 75;        // 75 by Default
    // HistNucSliceNumOfYBins = 75;        // 75 by Default
...
void AMaps::DrawAndSaveHitMaps(const string &SampleName, TCanvas *h1DCanvas, const string &AcceptanceMapsDirectory) {
    cout << "\n\nFillHistogramByProcessing maps...\n";
...
// LoadAMapLimits function ----------------------------------------------------------------------------------------------------------------------------------------------

//<editor-fold desc="LoadAMapLimits function">
void AMaps::LoadAMapLimits(const char *filename, vector<vector<double>> &Loaded_particle_limits) {
    ifstream infile;
    ...
    if (infile.is_open()) {
        ...
        cout << "\n\nAMaps::LoadBinaryAMapMatrices: file not found! Exiting...\n\n", exit(0);
    }
}
//</editor-fold>

// LoadAMapSlices function (AMaps) --------------------------------------------------------------------------------------------------------------------------------------

//<editor-fold desc="LoadAMapSlices function (AMaps)">
void AMaps::LoadAMapSlices(const string &SampleName, const string &AcceptanceMapsDirectory, const string &Particle, const vector<vector<double>> &Loaded_particle_limits,
    ...
    for (int Slice = 0; Slice < Loaded_particle_limits.size(); Slice++) {
        ...
        LoadBinaryAMapMatrices((AcceptanceMapsDirectory + SampleName + "/" + TempFileName).c_str(), Loaded_Particle_AMap_TempSlice);
...
// LoadBinaryAMapMatrices function (AMaps) --------------------------------------------------------------------------------------------------------------------------------------------

//<editor-fold desc="LoadBinaryAMapMatrices function (AMaps)">
/* A function that reads AMaps */

void AMaps::LoadBinaryAMapMatrices(const char *filename, vector<vector<int>> &Loaded_particle_AMap) {
    ifstream infile;
    infile.open(filename);
    ...
    if (infile.is_open()) {
        ...
        cout << "\n\nAMaps::LoadBinaryAMapMatrices: file:\n" << filename << "\nwas not found! Exiting...\n\n", exit(0);
    }
}
//</editor-fold>
